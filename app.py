...
def documentation_index():
    docs_path = os.path.join(app.root_path, 'mkdocs-docs', 'site')
...
def serve_documentation(path):
    ...
    # Handle directory requests (should serve index.html)
    if os.path.isdir(os.path.join(docs_path, path)):
        return send_from_directory(os.path.join(docs_path, path), 'index.html')
    
    # Handle all other files
    return send_from_directory(docs_path, path)
@app.route("/docs/assets/<path:filename>")
def serve_legacy_docs_assets(filename):
    assets_path = os.path.join(app.root_path, 'mkdocs-docs', 'site', 'assets')
